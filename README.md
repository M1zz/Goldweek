...
- **LeeoKit** - 공용 인프라 (피드백·익명 사용 통계·크래시 진단·원격 킬스위치·만족도 프롬프트)
- **CloudKit** - iCloud 백업
...
## 사용 통계·피드백 허브 (앱 안에서 보는 지표)

Firebase를 걷어낸 뒤, 앱이 값을 하는지 확인하는 유일한 경로다. 익명 사용 통계와 사용자 피드백,
크래시 진단을 공용 CloudKit 컨테이너(`iCloud.com.Ysoup.FeedbackHub`)에 쌓고 앱 안에서 읽는다.
새 외부 SDK는 없다(LeeoKit + CloudKit).

- 사용자에게 보이는 것: 설정 > 지원 — **피드백 보내기**, 리뷰 남기기, 개인정보·지원·약관 링크,
  그리고 조건이 맞을 때 뜨는 만족도 프롬프트("즐겁게 쓰고 계신가요?" → 아쉬우면 피드백 화면으로)
- 개발자만 보는 것(**버전 행 7번 탭**): 접수된 피드백 인박스 / 사용 통계(효용 지표·활성화 퍼널·
  리텐션 코호트·기간별 추이·이벤트별 사용량) / 안정성(MetricKit 크래시·멈춤을 버전별로)

⚠️ **CloudKit Dashboard 준비가 아직 남아 있다** (스키마 배포 + read 권한). 그전까지 통계·안정성 화면은
"권한 필요" 안내만 보여준다. 절차·수집 항목·심사 주의사항은 [docs/USAGE_STATS_HUB.md](docs/USAGE_STATS_HUB.md).
