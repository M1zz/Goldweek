...
class CalendarService {
    ...
    /// LeaveWise 전용 캘린더 찾기 또는 생성
    func getOrCreateLeaveWiseCalendar() throws -> EKCalendar {
        // 기존 LeaveWise 캘린더 찾기
        if let savedIdentifier = UserDefaults.standard.string(forKey: calendarIdentifierKey),
           let existingCalendar = eventStore.calendar(withIdentifier: savedIdentifier) {
            logDebug("기존 LeaveWise 캘린더 발견: \(existingCalendar.title)", category: .app)
            return existingCalendar
        }
        
        // 새 캘린더 생성
        let calendar = EKCalendar(for: .event, eventStore: eventStore)
        calendar.title = "LeaveWise 연차"
        ...
        logInfo("LeaveWise 캘린더 생성 완료: \(calendar.calendarIdentifier)", category: .app)
        return calendar
    }
    ...
    func addLeaveToCalendar(_ leaveRecord: LeaveRecord) async throws {
        ...
        let calendar = try getOrCreateLeaveWiseCalendar()
    ...
    /// LeaveWise 캘린더의 모든 이벤트 가져오기
    func getLeaveWiseEvents(from startDate: Date, to endDate: Date) throws -> [EKEvent] {
        guard isAuthorized else {
            throw CalendarError.permissionDenied
        }
    ...
    /// LeaveWise 캘린더 삭제 (설정 리셋용)
    func deleteLeaveWiseCalendar() throws {
        guard let savedIdentifier = UserDefaults.standard.string(forKey: calendarIdentifierKey),
            ...
            logInfo("삭제할 LeaveWise 캘린더가 없음", category: .app)
            return
        }
        ...
        logInfo("LeaveWise 캘린더 삭제 완료", category: .app)
    }
